...
def admin_settings_page():
    ...
    <div style="background: linear-gradient(135deg, #2c5aa0 0%, #5fa2e8 100%); color: white; padding: 1.5rem 2rem; border-radius: 12px; margin-bottom: 1.5rem; box-shadow: 0 4px 12px rgba(44, 90, 160, 0.2);">
        <div style="font-size: 1.6rem; font-weight: 700;">🛡️ System Administration</div>
        <div style="font-size: 0.85rem; opacity: 0.9;">Manage user access, approvals, and system lifecycle</div>
    </div>
    ...
    with tab1:
        ...
        for _, u in active.iterrows():
            with st.container():
                ...
                if c4.button("🗑️ Delete", key=f"del_u_{u['user_id']}", use_container_width=True):
                    if u['username'] == 'admin':
    ...
    with tab2:
        ...
        st.info("Assign specific project phases to personnel. Assigned staff will only see their own tasks on the 'Record Activity' page.")
        ...
        if not baseline_df.empty:
            ...
            if st.button("⚖️ Save Task Assignments"):
